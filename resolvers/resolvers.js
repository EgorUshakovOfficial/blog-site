...
const resolvers = {
    ...
    Mutation: {
        ...
        editPost: async (_, { postId, title, file, description }) => {
            ...
            let prevPhotoName = prevPost.photoUrl.replace(`${process.env.HOST}/images/`, '');
        ...
        deletePost: async (_, { postId }) => {
            ...
            // Then removes post from it
            let deletedPost = await Post.findOneAndRemove({ _id: postId });

            // Deletes photo from storage
            let photoName = deletedPost.photoUrl.replace(`${process.env.HOST}/images`, "");
            let currDirPath = process.cwd();
